...
import { DrizzleAlineacionRepo } from './Data/repos/alineacion.repo.js';
...
export async function registrarMetrics(app: FastifyInstance): Promise<void> {
  ...
  const alineaciones = new DrizzleAlineacionRepo();
  ...
  const captura = new CapturaService(acciones, sets, alineaciones, proyeccion);
